...
----- 1 Mengubah data nama departemen
DELIMITER //
CREATE PROCEDURE UpdateDataMaster(
    IN A_id INT,
    IN A_nama_departemen VARCHAR(100)
)
BEGIN
    UPDATE departemen
    SET nama_departemen = A_nama_departemen
    WHERE id_departemen = A_id;
...
----- pemanggilan -----
CALL UpdateDataMaster(1, 'Security 2');

----- 2 menjumlahkan kolom dalam tabel
DELIMITER //

CREATE PROCEDURE CountParkir(OUT total INT)
BEGIN
    SELECT COUNT(*) INTO total FROM parkir;
END //

DELIMITER ;

DROP PROCEDURE IF EXISTS CountParkir;

----- pemanggilan -----
SET @total = 0;
CALL CountParkir(@total);
SELECT @total;

----- 3 Menampilkan nama departemen by id
DELIMITER //

CREATE PROCEDURE GetDataDepartemenByID(
    IN id INT,
    OUT nama_departemen VARCHAR(100)
)
BEGIN
    SELECT departemenA.nama_departemen
    INTO nama_departemen
    FROM departemen AS departemenA
    WHERE departemenA.id_departemen = id;
END //
...
----- pemanggilan -----
SET @nama_departemen = '';
CALL GetDataDepartemenByID(1, @nama_departemen);
SELECT @nama_departemen;

----- 4 Update dua kolom dalam tabel karyawan
DELIMITER //

CREATE PROCEDURE UpdateFieldKaryawan(
    IN id INT,
    INOUT nama_karyawan_input VARCHAR(100),
    INOUT tgl_lahir_input DATE
)
BEGIN
    DECLARE current_nama_karyawan VARCHAR(100);
    DECLARE current_tgl_lahir DATE;

    -- Ambil data lama
    SELECT karyawanA.nama_karyawan, karyawanA.tgl_lahir
    INTO current_nama_karyawan, current_tgl_lahir
    FROM karyawan AS karyawanA
    WHERE karyawanA.id_karyawan = id;

    -- Update data di tabel karyawan
    UPDATE karyawan AS karyawanA
    SET karyawanA.nama_karyawan = nama_karyawan_input,
        karyawanA.tgl_lahir = tgl_lahir_input
    WHERE karyawanA.id_karyawan = id;
END //

DELIMITER ;

DROP PROCEDURE IF EXISTS UpdateFieldKaryawan;

----- pemanggilan -----
SET @nama_karyawan = 'Aldin Awaludin Din';
SET @tgl_lahir = '2008-08-16';
CALL UpdateFieldKaryawan(1, @nama_karyawan, @tgl_lahir);

----- 5 Menghapus data berdasarkan ID dari tabel master
DELIMITER //
...
BEGIN
    -- Menghapus data di tabel departemen berdasarkan ID
...
----- pemanggilan -----
CALL DeleteEntriesByIDMaster(6);

DELIMITER //
CREATE PROCEDURE tampil2(
    IN id_departemen_param INT,
    IN id_jabatan_param INT,
    OUT nama_departemen VARCHAR(100),
    OUT nama_jabatan VARCHAR(100)
)
BEGIN
    SELECT d.nama_departemen
    INTO nama_departemen
    FROM departemen d
    WHERE d.id_departemen = id_departemen_param;

    SELECT j.nama_jabatan
    INTO nama_jabatan
    FROM jabatan j
    WHERE j.id_jabatan = id_jabatan_param;
END //
DELIMITER ;

SET @nama_dep = '';
SET @nama_jab = '';
CALL tampil2(1, 2, @nama_dep, @nama_jab);
SELECT @nama_dep AS Departemen, @nama_jab AS Jabatan;
