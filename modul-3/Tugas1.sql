...
-- 1 Menambahkan data kategori UMKM
DELIMITER //

CREATE PROCEDURE AddUMKM (
    IN nama_usaha_baru VARCHAR(200),
    IN jumlah_karyawan_baru INT(11)
)
BEGIN
    INSERT INTO umkm (nama_usaha, jumlah_karyawan)
    VALUES (nama_usaha_baru, jumlah_karyawan_baru);
END //

DELIMITER ;
CALL AddUMKM('Shop Baby Mungil', 7);

DELIMITER //

-- 2 Mengubah data kategori UMKM
DELIMITER //

CREATE PROCEDURE UpdateKategoriUMKM (
    IN id_kategori_baru INT(11),
    IN nama_kategori_baru VARCHAR(100)
)
BEGIN
    UPDATE kategori_umkm
    SET nama_kategori = nama_kategori_baru
    WHERE id_kategori = id_kategori_baru;
END //

DELIMITER ;
CALL UpdateKategoriUMKM(12, 'Ekonomi Kreatif Banget');

-- 3 Menghapus pemilik UMKM by ID
DELIMITER //

CREATE PROCEDURE DeletePemilikUMKM (
    IN id_pemilik_baru INT
)
BEGIN
    DELETE FROM pemilik_umkm
    WHERE id_pemilik = id_pemilik_baru;
END //

DELIMITER ;
CALL DeletePemilikUMKM(15);

-- 4 Menambahkan data produk UMKM
DELIMITER //

CREATE PROCEDURE AddProduk (
    IN id_umkm_baru INT(11),
    IN nama_produk_baru VARCHAR(200),
    IN deskripsi_produk_baru TEXT,
    IN harga_produk_baru DECIMAL(15,2)
)
BEGIN
    INSERT INTO produk_umkm (id_umkm, nama_produk, deskripsi_produk, harga)
    VALUES (id_umkm_baru, nama_produk_baru, deskripsi_produk_baru, harga_produk_baru);
END //

DELIMITER ;
CALL AddProduk(14, 'Topi Rajutan Dewasa', 'Tpi rajutan untuk bayi', 30000.00);

-- 5 Mencari UMKM by ID
DELIMITER //
